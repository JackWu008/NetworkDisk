...
public class ModifyActivity extends BaseActivity {
    @BindView(R.id.activity_modify_edt_email)
    TextInputEditText edt_email;
    @BindView(R.id.activity_modify_til_email)
    TextInputLayout til_email;
    @BindView(R.id.activity_modify_btn_get_check_code)
    Button btn_get_code;
    @BindView(R.id.activity_modify_edt_check_code)
    TextInputEditText edt_check_code;
    @BindView(R.id.activity_modify_til_check_code)
    TextInputLayout til_check_code;
    @BindView(R.id.activity_modify_btn_modify)
    Button btn_modify;
    @BindView(R.id.activity_modify_edt_old_password)
    TextInputEditText edt_old_pas;
    @BindView(R.id.activity_modify_til_old_password)
    TextInputLayout til_old_pass;
    @BindView(R.id.activity_modify_edt_new_one)
    TextInputEditText edt_new_one;
    @BindView(R.id.activity_modify_til_new_one)
    TextInputLayout til_new_one;
    @BindView(R.id.activity_modify_edt_new_two)
    TextInputEditText edt_new_two;
    @BindView(R.id.activity_modify_til_new_two)
    TextInputLayout til_new_two;
    @BindView(R.id.activity_modify_toolbar)
    Toolbar toolbar;
    ...
    protected void init() {
        ...
        edt_old_pas.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length() < 6) {
                    til_old_pass.setError("密码的最小长度为6");
                    isOldPass = false;
                } else {
                    String pass1 = edt_new_one.getText().toString();
                    String pass2 = edt_new_two.getText().toString();
                    String pass3 = edt_old_pas.getText().toString();
                    if (pass1.equals(pass3) || pass2.equals(pass3)) {
                        isOldPass = false;
                        til_old_pass.setError("新密码不能和旧密码一样");
                    } else {
                        til_old_pass.setError("");
                        isOldPass = true;
                    }
                }
                check();
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length() < 6) {
                    ...
                    if (pass1.equals(pass2)) {
                        if (pass1.equals(pass3) || pass2.equals(pass3)) {
                            isNewTwo = false;
                            til_new_two.setError("新密码不能和旧密码一样");
                        } else {
    ...
    @OnClick({R.id.activity_modify_btn_get_check_code, R.id.activity_modify_btn_modify})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.activity_modify_btn_get_check_code:
                if (isEmail) {
                    btn_get_code.setEnabled(false);
                    btn_get_code.setBackgroundResource(R.drawable.btn_bg);
                    ...
                    til_email.setError("请输入正确的邮箱");
                break;
            case R.id.activity_modify_btn_modify:
                if (isEmail && isCode && isOldPass && isNewOne && isNewTwo) {
                    ...
                    if (NetworkUtils.isConnected()) {
                        ...
                        OkGo.<String>post(Constants.API_URL_MODIFY_PASSWORD)
                                .tag(Constants.API_URL_MODIFY_PASSWORD)
                                .params(Constants.API_EMAIL, edt_email.getText().toString())
                                .params(Constants.API_CODE, edt_check_code.getText().toString())
                                .params(Constants.API_OLD_PASSWORD, EncryptUtils.encryptMD5ToString(edt_old_pas.getText().toString()))
                                .params(Constants.API_NEW_PASSWORD, EncryptUtils.encryptMD5ToString(edt_new_two.getText().toString()))
                                .execute(new StringCallback() {
                                    @Override
                                    public void onStart(Request<String, ? extends Request> request) {
                                        super.onStart(request);
                                        KeyBoardUtil.closeKeybord(ModifyActivity.this);
                                        dialog.show();
                                    }
